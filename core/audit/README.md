...
Management **HTTP** routes, permission-aware reads, and response mapping are implemented in **`feature:user`**.
...
- For management HTTP APIs over audit data, depend on `feature:user` and install `UserModules` in addition to **[AuditModules]**.
