 ...
 *
 * - If the parameter with [name] is missing, throws [ValidationException] with [CommonError.MissingRequiredParameter].
 * - If [mapper] fails to convert the raw string, throws [ValidationException] with [CommonError.InvalidParameterValue].
 *
 * @param name name of the path parameter
 * @param mapper function that converts the raw string value to type [T]
 * @throws ValidationException when the parameter is missing or cannot be converted
 */
