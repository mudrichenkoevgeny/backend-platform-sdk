 ...
 * @throws ValidationException when a present parameter has an invalid value
 */
fun <SortBy> ApplicationCall.parseListingQueryParams(
    ...
    val sortBy = if (sortByRaw == null) {
        ...
        parseSortByOrNull(sortByRaw)
            ?: throw ValidationException(CommonError.InvalidParameterValue(ListingParamNames.Sort.SORT_BY))
    }
    ...
    val sortOrder = if (sortOrderRaw == null) {
        ...
        SortOrder.fromValueOrNull(sortOrderRaw)
            ?: throw ValidationException(CommonError.InvalidParameterValue(ListingParamNames.Sort.SORT_ORDER))
    }
