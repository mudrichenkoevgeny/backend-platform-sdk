...
class CommonWebSocketMessageHandlerTest {
    ...
    fun `PING produces PONG frame`() = runSuspend {
        ...
        val send = assertInstanceOf(WebSocketMessageHandlerResult.SendSocketFrame::class.java, result)
    ...
    fun `INITIALIZE without payload returns MissingRequiredField error`() = runSuspend {
        ...
        val error = assertInstanceOf(WebSocketMessageHandlerResult.Error::class.java, result)
        assertInstanceOf(CommonError.MissingRequiredField::class.java, error.appError)
    ...
    fun `INITIALIZE with invalid payload returns InvalidJsonBody error`() = runSuspend {
        ...
        val error = assertInstanceOf(WebSocketMessageHandlerResult.Error::class.java, result)
        assertInstanceOf(CommonError.InvalidJsonBody::class.java, error.appError)
    }
    ...
    fun `INITIALIZE with valid payload returns InitializeClient with success frame`() = runSuspend {
        val payload = WebSocketInitializePayload(
            clientType = UserClientType.WEB.serialName,
            language = "en",
            deviceId = "device-1",
            deviceName = "Pixel",
            appVersion = "1.2.3",
        ...
        val init = assertInstanceOf(WebSocketMessageHandlerResult.InitializeClient::class.java, result)
