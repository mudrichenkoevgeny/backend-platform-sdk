...
@Singleton
class ManagementDeleteIdentifierPasswordUseCase @Inject constructor(
    ...
    /**
     * Administratively revokes and removes the password credential from a specific user identifier
     * and terminates all sessions associated with it.
     *
     * **Allowed Account Statuses:** [UserAccountStatus.ACTIVE] for the management caller.
     *
     * **Security:**
     * - Requires an active management session (STAFF or ADMIN) with appropriate password deletion permissions.
     ...
     * - Prevents self-targeting via management routes.
     * - Protects against abuse via [UserManagementRateLimitAction.MANAGEMENT_IDENTIFIER_DELETE_PASSWORD].
     *
     * **Workflow:**
     ...
     * 4. Verifies that the identifier supports passwords and currently has a password credential set.
     ...
     * 7. Clears the password hash via [identifierManager].
     * 8. Terminates and sends a [UserWebSocketEventTypes.SESSION_DELETED] frame to all sessions linked
     *    to the identifier whose password was revoked.
     * 9. Logs the action via [AuditLogger] with [UserAuditActionType.MANAGEMENT_DELETE_IDENTIFIER_PASSWORD].
     *
     * @param userIdentifierId The ID of the identifier whose password is to be removed.
    ...
    suspend operator fun invoke(
        ...
        val rateLimitCheck = rateLimiter.checkRateLimit(
            action = UserManagementRateLimitAction.MANAGEMENT_IDENTIFIER_DELETE_PASSWORD,
        ...
        val requiredPermission = when (targetUser.role) {
            UserRole.USER -> IdentifierPermissionCode.IDENTIFIER_DELETE_PASSWORD_FOR_USER
            UserRole.STAFF -> IdentifierPermissionCode.IDENTIFIER_DELETE_PASSWORD_FOR_STAFF
        ...
        if (userIdentifier.userAuthProvider != UserAuthProvider.EMAIL) {
            return handleError(
                error = UserError.UserIdentifierPasswordNotSupported(),
                actorId = auditActorId,
                actorUserRole = auditActorUserRole,
                resourceId = auditResourceId,
                baseMetadata = auditMetadata
        ...
        if (userIdentifier.passwordHash == null) {
            return handleError(
                error = UserError.UserIdentifierPasswordNotSet(),
                actorId = auditActorId,
                actorUserRole = auditActorUserRole,
                resourceId = auditResourceId,
                baseMetadata = auditMetadata
        ...
        val sessionsForIdentifier = sessionManager.getUserSessionsByIdentifierId(
            userIdentifierId = userIdentifierId,
            userId = targetUser.id
        ).dataOrNull() ?: emptyList()

        val clearPasswordResult = identifierManager.clearUserIdentifierPassword(
            userIdentifier = userIdentifier
        )

        return when (clearPasswordResult) {
            is AppResult.Success -> {
                sessionsForIdentifier.forEach { session ->
                    sessionManager.deleteSessionById(session.id)
            ...
            is AppResult.Error -> handleError(
                error = clearPasswordResult.error,
                actorId = auditActorId,
                actorUserRole = auditActorUserRole,
                resourceId = auditResourceId,
                baseMetadata = auditMetadata
