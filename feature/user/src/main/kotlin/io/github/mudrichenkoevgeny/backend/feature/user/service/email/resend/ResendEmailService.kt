package io.github.mudrichenkoevgeny.backend.feature.user.service.email.resend
...
import io.github.mudrichenkoevgeny.backend.feature.user.service.email.resend.model.ResendEmailRequest
import io.github.mudrichenkoevgeny.backend.feature.user.service.email.resend.model.ResendErrorResponse
...
class ResendEmailService @Inject constructor(
    ...
    private suspend fun executeSend(email: String, subject: String, htmlContent: String): AppResult<Unit> {
        val baseUrl = config.url.trim().removeSurrounding("\"").removeSuffix("/")
        val fullUrl = "$baseUrl/emails"

        val from = "${config.fromName.trim().removeSurrounding("\"")} <${config.fromEmail.trim().removeSurrounding("\"")}>"
        ...
        return try {
            val response = httpClient.post(fullUrl) {
                header("Authorization", "Bearer ${config.apiKey.trim().removeSurrounding("\"")}")
            ...
            if (response.status.isSuccess()) {
                ...
                val errorBody = response.bodySafe<ResendErrorResponse>()
                val errorMessage = errorBody?.message?.takeIf { it.isNotBlank() }
                    ?: "Resend error (${response.status.value})"
